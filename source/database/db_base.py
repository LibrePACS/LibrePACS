...
def db_open(self, db_prod=True):
    ...
    if db_prod is True:
        self.sql_inst = psycopg2.connect("dbname='%s' user='%s' host='%s' port=%s password='%s'"
                                          % (os.environ['POSTGRES_DB'], os.environ['POSTGRES_USER'],
                                             'mkpgbounce', 6432, os.environ['POSTGRES_PASSWORD']))
    else:
        self.sql_inst = psycopg2.connect("dbname='librepacs' user='librepacs'"
                                          " host='th-postgresql-1' port=5432 password='librepacs'")
    self.sql_inst.set_isolation_level(ISOLATION_LEVEL_AUTOCOMMIT)
    self.db_cursor = self.sql_inst.cursor(
